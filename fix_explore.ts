...
const leafletHtmlMatch = content.match(/const leafletHTML = `(.*?)`;/s);
if (leafletHtmlMatch) {
...
const newInject = `const injectStoreData = () => {
  ...
  useEffect(() => { injectStoreData(); }, [stores, activeGateway]); // Push newly fetched stores immediately
  
  // ignore this to replace`;
...
console.log('Fixed explorer screen!');
