...
import { 
  MapPin, LayoutGrid, Map as MapIcon, Navigation, Loader2, ArrowRight 
} from "lucide-react";
  ...
  const handleGetLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          // Fetch stores within 10km
          const nearbyStores = await fetchStores(lat, lng, 10);
          setStores(nearbyStores);
          setViewMode("map"); // Switch to map view automatically
        } catch (e) {
          console.error(e);
        } finally {
          setLoading(false);
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-10 gap-4">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 flex items-center gap-3">
            ...
            {location ? "Stores Near You" : "Browse Stores"}
          </h2>
          <p className="text-slate-500 mt-1">
            {location 
              ? `Found ${stores.length} stores within 10km.` 
        ...
        <div className="flex items-center gap-3 bg-white/50 backdrop-blur-md p-1.5 rounded-2xl border border-white/40 shadow-sm">
          ...
          <button 
            ...
            className={`p-2.5 rounded-xl transition-all ${viewMode === 'grid' ? 'bg-white text-indigo-600 shadow-md' : 'text-slate-500 hover:bg-white/50'}`}
          >
            <LayoutGrid size={20} />
          </button>
          <button 
            onClick={() => setViewMode("map")}
            className={`p-2.5 rounded-xl transition-all ${viewMode === 'map' ? 'bg-white text-indigo-600 shadow-md' : 'text-slate-500 hover:bg-white/50'}`}
          >
            <MapIcon size={20} />
          </button>
        </div>
