...
import { Loader2, Save, LayoutTemplate, Palette, Megaphone, CheckCircle2, ShoppingBag, MapPin } from 'lucide-react'
import { api } from '@/lib/api'

// ✨ 8 ULTRA-PREMIUM THEMES ✨
const THEMES = [
  { id: 'apple-dark', name: 'Cupertino Dark', bg: '#000000', text: '#F5F5F7', primary: '#2997FF' }, // Apple Dark Mode
  { id: 'apple-light', name: 'Cupertino Light', bg: '#F5F5F7', text: '#1D1D1F', primary: '#0066CC' }, // Apple Light Mode
  { id: 'stripe', name: 'Fintech Slate', bg: '#0A2540', text: '#FFFFFF', primary: '#635BFF' }, // Stripe
  { id: 'luxury-gold', name: 'Maison Gold', bg: '#0A0A0A', text: '#FAFAFA', primary: '#D4AF37' }, // Luxury
  { id: 'minimal-mono', name: 'Pure Minimal', bg: '#FFFFFF', text: '#000000', primary: '#000000' }, // High Fashion
  { id: 'emerald', name: 'Emerald Isle', bg: '#022C22', text: '#ECFDF5', primary: '#10B981' }, // Rich Green
  { id: 'rose-glass', name: 'Blush Velvet', bg: '#FFF1F2', text: '#4C0519', primary: '#E11D48' }, // Cosmetics
  { id: 'oceanic', name: 'Deep Oceanic', bg: '#082F49', text: '#F0F9FF', primary: '#0EA5E9' }, // Modern Blue
]
...
export default function StoreSettingsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    } catch (err) { alert("Error saving.") } finally { setIsLoading(false) }
  }
  ...
  return (
    <main className="p-4 md:p-8 max-w-[1600px] mx-auto h-full flex flex-col lg:flex-row gap-8">
      ...
      <div className="flex-1 flex flex-col gap-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          ...
          <button onClick={handleSave} disabled={isLoading} className="mt-4 md:mt-0 bg-black text-white px-8 py-3 rounded-full font-bold hover:bg-gray-800 flex items-center gap-2 shadow-lg transition-all active:scale-95">
            {isLoading ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />} Publish Changes
          </button>
        </div>
      ...
      <div className="w-full lg:w-[420px] shrink-0">
        <div className="sticky top-8 bg-gray-200 rounded-[3.5rem] p-3.5 shadow-2xl border-[8px] border-gray-900 h-[820px] flex flex-col overflow-hidden relative">
          ...
          {/* Phone Screen */}
          <div className="flex-1 rounded-[2.8rem] overflow-y-auto no-scrollbar relative transition-colors duration-700" style={{ backgroundColor: formData.background_color, color: formData.secondary_color }}>
            ...
            {/* Hero Mock */}
            <div className="h-56 relative flex items-center justify-center m-2 rounded-3xl overflow-hidden">
               ...
               <div className="relative z-10 flex flex-col items-center">
                 <div className="w-16 h-16 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center mb-2 overflow-hidden shadow-xl border border-white/20">
                   {logoPreview ? <img src={logoPreview} className="w-full h-full object-cover" /> : <ShoppingBag size={20} className="text-white" />}
                 </div>
                 <h3 className="text-white font-black text-xl tracking-tight shadow-black/50 drop-shadow-md">New Arrivals</h3>
               </div>
            </div>
