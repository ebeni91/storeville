...
import { Loader2, ShoppingBag, MapPin, Search, Heart, User, SlidersHorizontal, ArrowRight, X, Check, Menu, Star, CheckCircle, Lock,Plus } from 'lucide-react'
...
export default function RetailStorefront({ store }: { store: any }) {
  ...
  useEffect(() => {
    ...
    // 2. Load Cart
    const savedCart = localStorage.getItem(`cart_retail_${store.id}`)
    if (savedCart) {
      try { setCartItems(JSON.parse(savedCart)) } catch (e) {}
    }

    // 3. Fetch New Retail Architecture
  ...
  useEffect(() => {
    if (store) localStorage.setItem(`cart_retail_${store.id}`, JSON.stringify(cartItems))
  }, [cartItems, store])
  ...
  const removeFromCart = (productId: string) => setCartItems(cartItems.filter(item => item.id !== productId))
      ...
      <nav className="sticky top-0 z-50 w-full backdrop-blur-2xl border-b transition-all duration-300 flex flex-col" style={{ backgroundColor: `rgba(${bgRgb}, 0.75)`, borderColor: `rgba(${textRgb}, 0.1)` }}>
        ...
        <div className="max-w-[1600px] w-full mx-auto px-4 md:px-8 py-4 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center bg-black/5 rounded-full px-5 py-2.5 w-[400px] border shadow-inner transition-all hover:shadow-md" style={{ borderColor: `rgba(${textRgb}, 0.1)`, backgroundColor: `rgba(${textRgb}, 0.03)` }}>
            ...
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search curated collection..." className="bg-transparent border-none outline-none px-3 w-full text-sm font-medium" style={{ color: store.secondary_color }} />
          </div>
